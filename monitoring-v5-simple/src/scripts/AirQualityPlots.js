...
export default class AirQualityPlot {
	...
	pm25_dailyBarplot(
		...
		for (let i = 0; i < plotParams.daily_avg_pm25.length; i++) {
			seriesData[i] = {
				y: plotParams.daily_avg_pm25[i],
				color: this.pm25ToColor(plotParams.daily_avg_pm25[i])
			};
		}

		let days = plotParams.daily_datetime.map((x) =>
			moment.tz(x, plotParams.timezone).format('MMM DD')
		);
	...
	pm25_diurnalPlot(
		...
		for (let i = 0; i < plotParams.yesterday.length; i++) {
			yesterdayData[i] = {
				y: plotParams.yesterday[i],
				color: this.pm25ToColor(plotParams.yesterday[i])
			};
		}
		let todayData = [];
